...
export default function AdminDashboard() {
  ...
  const loadDashboard = async () => {
    try {
      ...
      const [productsData, ordersData, customersData] =
        await Promise.all([
          fetchApi(`${API_URL}/products/`, token),
          fetchApi(`${API_URL}/orders/admin/`, token),
          fetchApi(`${API_URL}/users/admin/customers/`, token),
        ]);
...
const netSales = Math.max(
  ...
  const shippedOrders = useMemo(() => {
    return orders.filter((order) => {
      return getOrderStatus(order) === "shipped";
    }).length;
  }, [orders]);
  ...
  const topProducts = useMemo(() => {
    ...
    orders.forEach((order) => {
      ...
      items.forEach((item) => {
        ...
        productMap[productId].quantity += getItemQuantity(item);
  ...
  const revenueChart = useMemo(() => {
    ...
    paidOrders.forEach((order) => {
      ...
      days.forEach((day) => {
        if (
          orderDate.getFullYear() === day.date.getFullYear() &&
          orderDate.getMonth() === day.date.getMonth() &&
          orderDate.getDate() === day.date.getDate()
        ) {
          day.amount += getOrderAmount(order);
        }
      });
    });
      ...
      <section className="lg:ml-[250px] p-5 md:p-8">
        ...
        <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-5 mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* NOTIFICATIONS */}
            <button className="relative w-11 h-11 bg-white border border-gray-200 rounded-xl flex items-center justify-center hover:bg-gray-50">
              <Bell size={18} />
            ...
            {/* MESSAGES */}
            <button className="w-11 h-11 bg-white border border-gray-200 rounded-xl flex items-center justify-center hover:bg-gray-50">
  ...
  <StatCard
    title="Delivery Fees"
    value={formatCompactCurrency(deliveryFees)}
    icon={<Truck size={19} />}
  ...
  <StatCard
    title="Net Sales"
    value={formatCompactCurrency(netSales)}
    icon={<DollarSign size={19} />}
